...
import { HdPath, Secp256k1Signature } from "../crypto";
...
export class LedgerConnector {
  ...
  public constructor(
    ...
    this.testModeAllowed =
      options.testModeAllowed ?? defaultOptions.testModeAllowed;
  ...
  public async getPubkeys(): Promise<readonly Uint8Array[]> {
    return this.hdPaths.reduce(
      (promise: Promise<readonly Uint8Array[]>, hdPath) =>
        promise.then(async (pubkeys) => [
          ...pubkeys,
          await this.getPubkey(hdPath),
        ]),
      Promise.resolve([])
    );
  }
  ...
  public async sign(message: Uint8Array, hdPath?: HdPath): Promise<Uint8Array> {
    ...
    const response = await this.app.sign(
      unharden(hdPathToUse),
      fromUtf8(message)
    );
    this.handleLedgerErrors(
      response,
      "Transaction signing request was rejected by the user"
    );
    return Secp256k1Signature.fromDer(
      (response as SignResponse).signature
    ).toFixedLength();
  }
  ...
  private async verifyCosmosAppIsOpen(): Promise<void> {
    ...
    if (appName.toLowerCase() === `dashboard`) {
      throw new Error(
        `Please open the ${this.ledgerAppName} Ledger app on your Ledger device.`
      );
    }
  ...
  public async showAddress(hdPath?: HdPath): Promise<AddressAndPubkey> {
    ...
    const hdPathToUse = hdPath || this.hdPaths[0];
    // ledger-cosmos-js hardens the first three indices
    const response = await this.app.showAddressAndPubKey(
      unharden(hdPathToUse),
      this.prefix
    );
    this.handleLedgerErrors(response);
